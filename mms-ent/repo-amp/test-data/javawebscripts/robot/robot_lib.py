...
def write_json_to_file(json_object, filename):
    ...
    json_file = "output/original/{}_orig.json".format(filename.replace(' ', ''))
...
def filter_json_file(filename, filters_list):
    ...
    with open("output/original/{}_orig.json".format(filename), "r") as file_orig:
    ...
    filtered_filename = "output/results/{}.json".format(filename)
...
def compare_json(test_name):  # , json_object, filters):
    ...
    result_json = None
    try:
        with open("output/baseline/{}.json".format(test_name.replace(' ', ''))) as baseline_file:
            try:
...
def get_id_from_json(test_name, root_key):
    ...
    """
    json_object = open('output/original/{}_orig.json'.format(test_name))
...
def get_copy_time_from_json(test_name, root_key, exclude_origin=False):
    ...
    json_object = open('output/original/{}.json'.format(json_file))
    json_object = json.load(json_object)
    created = ''
