...
/**
 * Alfresco DocWeb Dashlet
 *
 * @namespace Alfresco.dashlet
 * @class Alfresco.dashlet.DocWeb
 */
(function()
{
   /**
   ...
   Alfresco.dashlet.DocWeb = function DocWeb_constructor(htmlId)
   {
      Alfresco.dashlet.DocWeb.superclass.constructor.call(this, "Alfresco.dashlet.DocWeb", htmlId);
   ...
   YAHOO.extend(Alfresco.dashlet.DocWeb, Alfresco.component.Base,
   {
      /**
       ...
       * @method onReady
       */
      onReady: function DocWeb_onReady()
      {
         /**
      ...
      {
         Dom.removeClass(this.id, "docweb-default");
         Dom.removeClass(this.id, "docweb-notsecure");
         Dom.removeClass(this.id, "docweb-iframe");

         if (this.options.isDefault)
         {
            Dom.addClass(this.id, "docweb-default");
         }
         else if (!Alfresco.util.IFramePolicy.isUrlAllowed(this.options.docwebURI))
         {
            Dom.addClass(this.id, "docweb-notsecure");
         }
         else
         {
            // Iframe itself
            this.widgets.iframe.src = this.options.docwebURI;

            // Title link
            this.widgets.iframeTitle.href = this.options.docwebURI;

            // Title label
            if (this.options.docwebTitle != "")
            {
               this.widgets.iframeTitle.innerHTML = $html(this.options.docwebTitle);
            }
            else if (!this.options.isDefault)
            {
               this.widgets.iframeTitle.innerHTML = $html(this.options.docwebURI);
            }
            Dom.addClass(this.id, "docweb-iframe");
         }
      },
       ...
       *
       * @method onConfigDocWebClick
       * @param e {object} HTML event
       */
      onConfigDocWebClick: function DocWeb_onConfigDocWebClick(e)
      {
         Event.stopEvent(e);
         
         var actionUrl = Alfresco.constants.URL_SERVICECONTEXT + "modules/docweb/config/" + encodeURIComponent(this.options.componentId);
         ...
         if (!this.configDialog)
         {
            ...
            {
               width: "50em",
               templateUrl: Alfresco.constants.URL_SERVICECONTEXT + "modules/docweb/config",
               onSuccess:
               {
                  fn: function DocWeb_onConfigDocWeb_callback(response)
                  {
                     ...
                     else
                     {
                        ...
                        this.options.docwebURI = data.uri;
                        this.options.docwebTitle = data.title;
               ...
               {
                  fn: function DocWeb_doSetupForm_callback(form)
                  {
                     ...
                     /* Get the link title */
                     var elem = Dom.get(this.configDialog.id + "-docwebTitle");
                     if (elem)
                     {
                        elem.value = this.options.docwebTitle;
                     }
                     ...
                     if (elem)
                     {
                        elem.value = this.options.isDefault ? "" : this.options.docwebURI;
                     }
                  },
                  scope: this
               }
